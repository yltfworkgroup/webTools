 ...
 */
function initWebTree(id,arr,userOption = {}){
...
function getWebTreeLiHtml(arr){
    ...
    for(let i=0;i<arr.length;i++){
        ...
        if(arr[i][option.child] != undefined){
            lihtml += '     <span class="tree-title" onclick="spanClick(this)"><i class="tree-icon"></i><a onclick="webTreeAclick(this,event)">' + arr[i][option.label] + '</a><i style="display:none">' + arr[i][option.value] + '</i></span>';
        }else{
            lihtml += '     <span class="tree-title" onclick="spanClick(this)"><i class="tree-nochild"></i><a onclick="webTreeAclick(this,event)">' + arr[i][option.label] + '</a><i style="display:none">' + arr[i][option.value] + '</i></span>';
        }
        if(arr[i][option.child] != undefined){
            lihtml += '<ul class="tree-list" style="min-width:' + option.width + '">';
